...
write.csv(ctrs, "~/dropbox/uurabo/nwcountries2.csv")
# iso2c <- substr(ctrs$iso3c, 1, 2)
...
air_pm10_nw[, mean(statistic_value), by=list(ctr, statistics_year, statistic_name)][, range(statistics_year)]
...
air_pm10_nw[statistic_name=="annual mean", mean(statistic_value), by=list(ctr, statistics_year, statistic_name)][, range(statistics_year)]
